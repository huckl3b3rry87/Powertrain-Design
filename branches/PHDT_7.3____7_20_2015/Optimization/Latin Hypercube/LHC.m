...
for nn = 1:n
    ...
    [ PASS_1,  ~] = Acceleration_Time_Check( Sim_Variables, test.V_0_n,test.V_f_n, test.dt_2 );
    
    % performance check for selected drive cycles
    load V_0.mat
    load V_f.mat
    load Acc_Final.mat
    [ PASS_2,~] = Max_Acceleration(V_0,V_f, Acc_Final, Sim_Variables);
    
    Pass_Total = [PASS_1,PASS_2];
    fail_acc_test = ~Pass_Total;
    ...
    %--------------------------Set Requirements--------------------------------
    Motor_ON = 1;  %
    [Sim_Grade, FAIL_GRADE_TEST] = Grade_Test( param, vinf, dvar, test.alpha, test.V, Motor_ON );
