...
% update engine weight based off of new engine torque scaling factor,
% defining a temporary variable for this function
ENG_WEIGHT = weight.engine_event*dvar.fc_trq_scale;  % cannot update structure, because it will continuously scale down ( could fix like the fuel tables..)
...
for t = 1:cyc_data.time_cyc
    ...
    infeasible_We = single(zeros(x3_length,u1_length,u2_length));        % [x3]x[u1]x[u2]
    table_x1 = single(zeros(x1_length,x3_length,u1_length,u2_length,u3_length));   
    ...
    table_x1_eng_on = single(zeros(x1_length,x3_length,u1_length,u2_length));   % [x1]x[x3]x[u1]x[u2]
    table_x1_eng_idle_off = single(zeros(x1_length,x3_length,u1_length,u2_length));   % [x1]x[x3]x[u1]x[u2]
    ...
    for x3 = 1:x3_length           % go through all of the gears
        ...
        for u2 = 1:u2_length       % shift down, don't shift, and shift up
            ...
            if cyc_data.Ww(t)~=0
                ...
                infeasible_We(x3,:,u2) = zeros(size(u1_grid));

            else           % if the vehicle is stopped, the engine cannot charge the motor
                infeasible_Te(x3,:,u2) = (0 < Te_c);                            % [u1]x[1]
               
                % dissuade engine from trying to run the engine ( eng state 2 ) while the vehicle is stopped, in the past it was able to do this if it applies no engine torque
                infeasible_We(x3,:,u2) = ones(size(u1_grid));
            end
            ...
            if RUN_TYPE.emiss_data == 1
                ...
                % for idling       
                inst_fuel_idle(x3,1,u2) = weight.fuel*vinf.fuel_idle + weight.NOx*vinf.NOx_idle + weight.CO*vinf.CO_idle + weight.HC*vinf.HC_idle + Shift_Penalty;
            else
                inst_fuel(x3,:,u2) = weight.fuel*fuel + Shift_Penalty*ones(size(fuel));
                
                % for idling
                inst_fuel_idle(x3,1,u2) = weight.fuel*vinf.fuel_idle + Shift_Penalty;
            end
            ...
            % saturate the motor torque, can make up the rest in braking
            Tm_actual = Tm_c;
            Tm_actual(Tm_c < Tm_min_current) =  Tm_min_current(Tm_c < Tm_min_current);
            
            % update x1
            Pbat_charge = (Wm_c*Tm_actual).*(eff_m*vinf.ess_coulombic_eff);    % Tm_c < 0
            Pbat_discharge = (Wm_c*Tm_actual)./(eff_m*vinf.ess_coulombic_eff); % battery needs to supply more power!!
            ...
            table_x1_eng_on(:,x3,:,u2) = SOC_n;
            table_x1_eng_idle_off(:,x3,:,u2) = repmat(SOC_n(:,1),[1,u1_length]); % [x1]x[x3]x[u1]x[u2]
    ...
    infeasible_We = (We_save < vinf.We_fail) | (We_save > vinf.W_eng_max) + infeasible_We;
    ...
    % note: SOC tables are not affected by what it was, they will be affected by the engine torque hense engine control control appled at that time step though
    table_x1(:,:,:,:,1) =  table_x1_eng_idle_off;  % [x1]x[x3]x[u1]x[u2]x[ 1 for u3]
    table_x1(:,:,:,:,2) =  table_x1_eng_on;
    table_x1(:,:,:,:,3) =  table_x1_eng_idle_off;  % [x1]x[x3]x[u1]x[u2]x[ 1 for u3]
    
    % check SOC
    infeasible_SOC = (table_x1 < param.MIN_SOC) | (table_x1 > param.MAX_SOC);
    infeasible_SOC = repmat(infeasible_SOC,[1,1,1,1,1,x2_length]);
    infeasible_SOC = permute(infeasible_SOC,[1,6,2,3,4,5]);
    ...
    SOC_soft = repmat(SOC_soft,[1,1,1,1,1,x2_length]);
    SOC_soft = permute(SOC_soft,[1,6,2,3,4,5]);
    ...
    inst_fuel_off = repmat(inst_fuel_off,[1,u1_length,1,x1_length,1,1]);   % [x3{1}] x [u1{2}] x [u2{3}] x [x1{4}] x [1-for x2{5}] x [1-for u3{6}]
    inst_fuel_off = permute(inst_fuel_off,[4 5 1 2 3 6]);
    inst_fuel(:,1,:,:,:,1) =  inst_fuel_off;      % still has shifting penalties
    inst_fuel(:,2,:,:,:,1) =  inst_fuel_off;      % 7/5/2015 - changed it from  inst_fuel(:,2,:,1,:,1) to inst_fuel(:,2,:,:,:,1) - with no effect...
    inst_fuel(:,3,:,:,:,1) =  inst_fuel_off;
    
    % add in engine idling results 
    inst_fuel_idle = repmat(inst_fuel_idle,[1,u1_length,1,x1_length,1,1]);  % [x3{1}] x [u1{2}] x [u2{3}] x [x1{4}] x [1-for x2{5}] x [1-for u3{6}]
    inst_fuel_idle = permute(inst_fuel_idle,[4 5 1 2 3 6]);
    inst_fuel(:,1,:,:,:,3) =  inst_fuel_idle;
    inst_fuel(:,2,:,:,:,3) =  inst_fuel_idle;
    inst_fuel(:,3,:,:,:,3) =  inst_fuel_idle;
    
    % penalize for using engine torque when engine is off
    infeasible_Te(:,1,:,(2:end),:,1) = ones(size(infeasible_Te(:,1,:,(2:end),:,1)))+ infeasible_Te(:,1,:,(2:end),:,1);
    infeasible_Te(:,2,:,(2:end),:,1) = ones(size(infeasible_Te(:,2,:,(2:end),:,1)))+ infeasible_Te(:,2,:,(2:end),:,1);
    infeasible_Te(:,3,:,(2:end),:,1) = ones(size(infeasible_Te(:,3,:,(2:end),:,1)))+ infeasible_Te(:,3,:,(2:end),:,1);
    
    % penalize for using engine torque when engine is idling
    infeasible_Te(:,1,:,(2:end),:,3) = ones(size(infeasible_Te(:,1,:,(2:end),:,3)))+ infeasible_Te(:,1,:,(2:end),:,3);
    infeasible_Te(:,2,:,(2:end),:,3) = ones(size(infeasible_Te(:,2,:,(2:end),:,3)))+ infeasible_Te(:,2,:,(2:end),:,3);
    infeasible_Te(:,3,:,(2:end),:,3) = ones(size(infeasible_Te(:,3,:,(2:end),:,3)))+ infeasible_Te(:,3,:,(2:end),:,3);
    
    % penalize for turning engine on
    inst_fuel(:,1,:,:,:,2) = ENG_WEIGHT*ones(size(inst_fuel(:,1,:,:,:,2))) + inst_fuel(:,1,:,:,:,2);
    inst_fuel(:,1,:,:,:,3) = ENG_WEIGHT*ones(size(inst_fuel(:,1,:,:,:,3))) + inst_fuel(:,1,:,:,:,3);
    
    % penalize for using clutch
    inst_fuel(:,2,:,:,:,3) = weight.clutch_event*ones(size(inst_fuel(:,2,:,:,:,3))) + inst_fuel(:,2,:,:,:,3);
    
    % do not penalize engine speed when it is off and it is not applying
    % any torque
    ...
    % do not penalize engine speed when it is idling and it is not applying
    % any torque
    infeasible_We(:,1,:,:,:,3) = zeros(size(infeasible_We(:,1,:,:,:,3)));
    infeasible_We(:,2,:,:,:,3) = zeros(size(infeasible_We(:,2,:,:,:,3)));
    infeasible_We(:,3,:,:,:,3) = zeros(size(infeasible_We(:,3,:,:,:,3)));
...
for t = cyc_data.time_cyc:-1:1
    ...
    load(loadfile_name);
    table_x1 = repmat(table_x1,[1,1,1,1,1,x3_length]); % add in extra dimension for engine state
    table_x1 = permute(table_x1,[1,6,2,3,4,5]);
...
for t = 1:1:cyc_data.time_cyc
    ...
    % shifting control
    id_lookup_u2 = interp1(x1_grid,opt_id_u2(:,x2,x3),SOC_c,'nearest');  % could use index extraction to speed up!!
    ...
    if (x2 == 1 && eng_c == 2) || (x2 == 1 && eng_c == 3); ENG_event = 1;  else ENG_event = 0; end    % to quantify how many times engine was turned on from off
    if x2 == 2 && eng_c == 3; CLUTCH_event = 1; else CLUTCH_event = 0; end                            % to quantify how many times clutch was engaged
    ...
    % final engine torque check
    if  Fail_Te ~= 1           % if it has not already failed
        if (cyc_data.Ww(t)==0 && Te_c ~= 0) 
            Fail_Te = 1;       % engine cannot provide torque when the vehicle is stopped
        else
            Fail_Te = 0;
        end
    end
    ...
    sim.CLUTCH_Event(t) = CLUTCH_event;
