...
hold on;
plot(vinf.m_map_spd*rads2rpm,vinf.m_max_trq,  'k','linewidth',k)
hold on
plot(vinf.m_map_spd*rads2rpm,vinf.m_max_gen_trq, 'k','linewidth',k)
...
A = [vinf.m_max_gen_trq'; sim.T_mot'];
B = [vinf.m_max_trq'; sim.T_mot'];
ylim([min(A)*1.1 max(B)*1.1])
