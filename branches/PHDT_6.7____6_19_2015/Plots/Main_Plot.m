...
set(AX(2),'YLim',[min(sim.SOC_final)*0.95 max(sim.SOC_final)*1.05]);
H = legend('Drive Profile','SOC'); set(H,'fontsize', h)
set(gcf, 'units', 'inch', 'pos', [0.13 0.778029445073613 0.775 0.146970554926388]);

subplot(5,1,2);
[AX,L1,L2] = plotyy(cyc_data.cyc_time,sim.inst_fuel,[cyc_data.cyc_time',cyc_data.cyc_time'],[sim.ENG_state',sim.clutch']);
set(L1,'marker','x', 'markersize', n, 'markerf','b','linewidth',q);
set(L2,'marker','x', 'markersize', n, 'markerf','g','linewidth',q);
set(AX, 'fontsize', y,'fontweight','bold','XTickLabel',[]);
ylabel({'Fuel Rate', '( g/s)'}, 'parent', AX(1));
ylabel({'(on/off)'}, 'parent', AX(2));grid
set(AX(1),'XLim',[0 t]);
set(AX(2),'XLim',[0 t]);
set(AX(1),'YLim',[-0.1 1.2*max(sim.inst_fuel)]);
set(AX(2),'YLim',[-0.1 3.1]);
H = legend('Fuel Rate','Engine State','Clutch Engagement');set(H,'fontsize', h)
...
subplot(5,1,4);
GRAPH =  plot(cyc_data.cyc_time,sim.T_brake/10, 'k',cyc_data.cyc_time,sim.T_mot,'g',cyc_data.cyc_time,sim.T_eng,'b'); grid on;
set(GRAPH,'marker','x', 'markersize', n, 'markerf', 'b','linewidth',q);
H=legend('(Brake Torque)/10','Motor Torque','Engine Torque');set(H,'fontsize', h)
ylabel({'Torque' ,'(Nm)'},'fontWeight','bold','fontSize',y)
set(gca,'fontSize',y,'fontWeight','bold');   
set(gca, 'pos',[0.129559619306594 0.255522536806342 0.775 0.1495]);
set(gca,'XTickLabel',[]);
END = [max(sim.T_mot), max(sim.T_eng), max(sim.T_brake)];
TRQ_min = [min(sim.T_mot), min(sim.T_brake/10)];
ylim([(min(TRQ_min)-5) 1.1*(max(END))]);
xlim([0 t]);
