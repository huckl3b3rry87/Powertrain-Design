...
% Penalize an Engine Event
eng_pen = [0; weight.engine_event*ones((length(u1_grid)-1),1)];
eng_pen = repmat(eng_pen,[1, x1_length, x3_length, u2_length]);
eng_pen = permute(eng_pen,[2 3 1 4]);
Eng_Penalty = single(zeros(x1_length,x2_length, x3_length,u1_length,u2_length));
Eng_Penalty(:,1,:,:,:) = eng_pen;
...
for t = 1:cyc_data.time_cyc
    ...
    for x3 = 1:x3_length           % go through all of the gears
        ...
        for u2 = 1:u2_length       % shift down, don't shift, and shift up
            ...
            if cyc_data.Ww ~= 0  % the vehicle is not stopped
                We_c = cyc_data.Ww(t)*dvar.FD*x3_n;                % [rad/sec]
            else
                We_c = vinf.W_eng_opt; % allow the engine the opportunity to run when vehicle is stopped
            end
            ...
            % Check Engine
            We_save(x3,:,u2) = [0; We_c*ones((length(u1_grid)-1),1)];                         % [u1]x[1]  - Check For Each Gear - Speed of the engine depends on the gear and engine state
            ...
            infeasible_Pbatt(:,x3,:,u2) = (Pbatt_max < Pbat); % Do not penlize it for operating too low, we can brake
    ...
    % Check Engine
%     infeasible_We = (We_save < 550*param.rpm2rads) | (We_save >
%     vinf.W_eng_max);  %  do not let engine opperate at speeds below a
%     particular value -- if this is re-implemented, it needs to not
%     penalize a zero engine speed
    infeasible_We = (We_save > vinf.W_eng_max);   % allow engine to opperate at low speeds without penalty
    ...
    table_L = inst_fuel + Eng_Penalty + SOC_soft + weight.infeasible*(infeasible_SOC + infeasible_We + infeasible_Tm + infeasible_Wm + infeasible_Te + infeasible_Pbatt);   %[x1]x[x2]x[u1]x[u2]x[u3]
...
cd(tables);      % Go into the tables that were previously made
...
for t = 1:1:cyc_data.time_cyc
    ...
    if Te_c ~= 0 && cyc_data.Ww(t) ~= 0 % the engine is on and the vehicle is not stopped
        We_c = cyc_data.Ww(t)*dvar.FD*x3_n; % [rad/sec]
    elseif Te_c ~= 0 % the engine is on and the vehicle is stopped
        We_c = vinf.W_eng_opt; % allow the engine the opportunity to run when vehicle is stopped
    else
        We_c = 0;  % eng is off
    end
    Te_drive = Te_c;   % If there is auxiliarry power, we subtract it here
    Wm_c = cyc_data.Ww(t)*dvar.FD*dvar.G;              % [rad/sec]
    ...
    %     if (Te_c ~= 0 && ((We_c < 550*param.rpm2rads) || We_c >
%     vinf.W_eng_max))% do not let the engine opperate below a particular
%     speed, !!! if this is changed back then it needs to be modified so
%     that the sim is not failed if the engine is off
    if (Te_c ~= 0 &&  We_c > vinf.W_eng_max)  % let engine opperate at low speeds
        Fail_We = 1;
    else
        Fail_We = 0;
    end
    ...
    elseif Tm_c < Tm_min;
        ...
        Tm_eff = Tm_min;
        Tm_actual = Tm_min;
        T_b = (Tm_c - Tm_actual)*(dvar.FD*dvar.G) ;  % make up the rest with braking torque, remember the motor torque was amplified
    else
        Fail_Tm = 0;
        Tm_eff = Tm_c;
        Tm_actual = Tm_c;
