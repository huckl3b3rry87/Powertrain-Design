function [ V_0, V_f,  Acc_Final] = Accel_Req_High( S, dt_2, graph )
...
for n = 1:5
    ...
    V_max = 85*mph_mps;
    low = [0,1,2,3,4,5]*mph_mps;
    V_test = [low,linspace(6*mph_mps,V_max,S-length(low))];
    ...
    for i = 1:S-1
        ...
        if ~isempty(Index)
            a_req(i) = abs(max(a(Index)));
        elseif i~=1 && ~(V_test(i+1) > max(v))
    ...
    if n~=3  && n~= 4% Exclude US06 and LA92 
        if n>3      
            a_save(n-1,:) = a_req;
        else
            a_save(n,:) = a_req;
