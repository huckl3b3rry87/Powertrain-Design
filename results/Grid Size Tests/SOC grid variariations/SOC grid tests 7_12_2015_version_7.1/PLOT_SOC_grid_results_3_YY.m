clear;
clf;
close all;
C = jet;
h = 14;
r=6;
t = 10;
q = 40;  % For colors
n = 4;
h = 9;
y = 12;
x = 1.25;

start = 4;
stop = 10;
for i = [start];
    if i == 1
        load SOC_grid_0_1_trq_1_Nm;
        c = 'SOC grid size = 0.1';
    elseif i == 2
        load SOC_grid_0_2_trq_1_Nm;
        c = 'SOC grid size = 0.2';
    elseif i == 3
        load SOC_grid_0_05_trq_1_Nm;
        c = 'SOC grid size = 0.05';
    elseif i == 4
        load SOC_grid_0_025_trq_1_Nm;
        c = 'SOC grid size = 0.025';
    elseif i == 5
        load SOC_grid_001_trq_1_Nm_HWFET;
        c = 'SOC grid size = 0.001';
    elseif i == 6
        load SOC_grid_005_interp;
        c = '0.005 lin. interp';
    elseif i == 7
        load SOC_grid_001_interp;
        c = '0.001 lin. interp.';
    elseif i == 8
        load SOC_grid_0005_interp;
        c = '0.0005 lin. interp.';
    elseif i == 9
        load SOC_grid_005_interp_cubic;
        c = '0.005 cubic interp.';
    elseif i == 10
        load SOC_grid_0005_interp_cubic;
        c = '0.0005 cubic. interp.';
    elseif i == 11
        load SOC_grid_005_interp_full_cyc;
        c = '0.005 line. interp FULL';
    elseif i == 12
        load SOC_grid_005_interp_full_cyc_smooth;
        c = '0.005 line. interp FULL Smoothed Data';
    else
        load SOC_grid_001_interp_full_cyc_smooth;
        c = '0.001 line. interp FULL Smoothed Data';
    end
    
    if i == start
        leg = {c};
        a = 1;
    else
        leg = {leg{1:a}, c};
        a = a+1;
    end
    
    figure(1)
    [AX,L1,L2] = plotyy(result.a1(1,:),result.mpg(1,:),result.a1(1,:),result.NOx(1,:));
    set(L1,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a+q,:),...
        'MarkerSize',r);
    set(L2,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a,:),...
        'MarkerSize',r);
    set(AX, 'fontsize', y,'fontweight','bold');
    ylabel({'MPG'}, 'parent', AX(1));
    ylabel('NOx (g)', 'parent', AX(2));grid
    xlabel('NOx weights')
    title(c);
    H = legend('MPG', 'NOx'); set(H,'fontsize', h)
    
    figure(2)
    [AX,L1,L2] = plotyy(result.a2(2,:),result.mpg(2,:),result.a2(2,:),result.CO(2,:));
    set(L1,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a+q,:),...
        'MarkerSize',r);
    set(L2,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a,:),...
        'MarkerSize',r);
    set(AX, 'fontsize', y,'fontweight','bold');
    ylabel({'MPG'}, 'parent', AX(1));
    ylabel('CO (g)', 'parent', AX(2));grid
    xlabel('CO weights')
    title(c);
    H = legend('MPG', 'CO'); set(H,'fontsize', h)
    
    figure(3)
    [AX,L1,L2] = plotyy(result.a3(3,:),result.mpg(3,:),result.a3(3,:),result.HC(3,:));
    set(L1,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a+q,:),...
        'MarkerSize',r);
    set(L2,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a,:),...
        'MarkerSize',r);
    set(AX, 'fontsize', y,'fontweight','bold');
    ylabel({'MPG'}, 'parent', AX(1));
    ylabel('HC (g)', 'parent', AX(2));grid
    xlabel('HC weights')
    title(c);
    H = legend('MPG', 'HC'); set(H,'fontsize', h)
    
    figure(4)
    [AX,L1,L2] = plotyy(result.shift(4,:),result.mpg(4,:),result.shift(4,:),result.SE(4,:));
    set(L1,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a+q,:),...
        'MarkerSize',r);
    set(L2,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a,:),...
        'MarkerSize',r);
    set(AX, 'fontsize', y,'fontweight','bold');
    ylabel({'MPG'}, 'parent', AX(1));
    ylabel('Total Shift Events', 'parent', AX(2));grid
    xlabel('Shift Event Weights')
    title(c);
    H = legend('MPG', 'SE'); set(H,'fontsize', h)

  figure(5)
    [AX,L1,L2] = plotyy(result.eng(5,:),result.mpg(5,:),result.eng(5,:),result.EE(5,:));
    set(L1,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a+q,:),...
        'MarkerSize',r);
    set(L2,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a,:),...
        'MarkerSize',r);
    set(AX, 'fontsize', y,'fontweight','bold');
    ylabel({'MPG'}, 'parent', AX(1));
    ylabel('Total Engine Events', 'parent', AX(2));grid
    xlabel('Engine Event Weights')
    title(c);
    H = legend('MPG', 'EE'); set(H,'fontsize', h)
    
    figure(6)
    [AX,L1,L2] = plotyy(result.clutch(6,:),result.mpg(6,:),result.clutch(6,:),result.CE(6,:));
    set(L1,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a+q,:),...
        'MarkerSize',r);
    set(L2,'marker','o',...
        'LineWidth',0.5,...
        'MarkerEdgeColor','k',...
        'MarkerFaceColor',C(a,:),...
        'MarkerSize',r);
    set(AX, 'fontsize', y,'fontweight','bold');
    ylabel({'MPG'}, 'parent', AX(1));
    ylabel('Total Idle Events', 'parent', AX(2));grid
    xlabel('Idle Event Weights')
    title(c);
    H = legend('MPG', 'Idle Events'); set(H,'fontsize', h)
end
hold off
set(findall(gcf,'type','text'),'FontSize',15,'fontWeight','bold')

